...
namespace JEngine {
    namespace Data {
        ...
        constexpr uint32_t updateCRC(uint32_t crc, const void* data, size_t length) {
        ...
        template<typename T>
        constexpr uint32_t calcualteCRC(const T* data, size_t length) {
            static_assert("Type not supported for constexpr CRC!");
        }

        template<>
        constexpr uint32_t calcualteCRC<char>(const char* data, size_t length) {
            uint32_t crc = 0xFFFFFFFFU;
            for (size_t i = 0; i < length; i++) {
                crc = detail::CRC_TABLE[(crc ^ uint32_t(data[i])) & 0xFF] ^ (crc >> 8);
            }
            return crc ^ 0xFFFFFFFFU;
        }

        template<>
        constexpr uint32_t calcualteCRC<uint8_t>(const uint8_t* data, size_t length) {
            uint32_t crc = 0xFFFFFFFFU;
            for (size_t i = 0; i < length; i++) {
                crc = detail::CRC_TABLE[(crc ^ data[i]) & 0xFF] ^ (crc >> 8);
            }
            return crc ^ 0xFFFFFFFFU;
        }
