...
namespace JEngine {
    ...
    public:
        ...
        IRenderer() : _color(255, 255, 255, 255), _layer(0, 0), _flags(REND_IS_VISIBLE), _bounds() {
        ...
        void setSortingOrder(int32_t order) { _layer.setOrder(order); }

        void setSortingLayer(std::string_view layerName) { _layer.setLayerByName(layerName); }
        void setSortingLayer(uint16_t layer) { _layer.setLayerByIndex(layer); }
        void setSortingLayer(SortingLayer layer) { _layer = layer; }

        SortingLayer getSortingLayer() const { return _layer; }

        virtual bool canRender() const { return bool(_flags & REND_IS_VISIBLE); }

        void setVisible(const bool visible) { _flags.setBit(REND_IS_VISIBLE, visible); }
        ...
        virtual const JVertex* getVertices() const = 0;
    ...
    protected:
        ...
        static void registerRenderer(IRenderer* rend) {
            if (REGISTER_RENDERER) {
                REGISTER_RENDERER(rend);
            }
        }
        static void unregisterRenderer(IRenderer* rend) {
