...
namespace JEngine {
    ...
    const int32_t sortRenderers(IRenderer2D* lhs, IRenderer2D* rhs) {
        ...
        auto matA = lhs->getMaterial();
        auto matB = rhs->getMaterial();
    ...
    Renderer::Renderer() : _batch(), _window(), _tick(), _initialized(false), _blendMaterial(nullptr) {
        if (!_instance) {
            _instance = this;
    ...
    const bool Renderer::init(const char* title, const int32_t width, const int32_t height) {
        if (_initialized) {
    ...
    const bool Renderer::doRender() {
        ...
        for (size_t i = 0; i < 33; i++) {
            std::sort(_activeRenderers[i].begin(), _activeRenderers[i].end(), sortRenderers);
        }
        ...
        for (std::pair<ICamera*, RenderInfoGroup> cam : _camerasToDraw) {
            if (cam.first->getLayerMask()) {
                cameras.push_back(cam.first);
            }
        }
        std::sort(cameras.begin(), cameras.end(), sortCameras);
        ...
        std::sort(_activeImGuiDrawables.begin(), _activeImGuiDrawables.end(), sortGuiDrawables);
    ...
    void Renderer::renderNull(RenderInfo& renderInfo, RenderInfo& uiRenderInfo) {
        ...
        if (matBlend && matBlend->getShaderPtr()) {
            ...
            _batch.addVerts(mat, JColor32::White, QUAD, 4, QUAD_INDICES, 6);
        ...
        blit:
            _window.updateViewport(dstMin, dstSiz);
            GLCall(glBindFramebuffer(GL_READ_FRAMEBUFFER, fBuffer.getBufferId()));
            GLCall(glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0));
            GLCall(glBlitFramebuffer(
                srcMin.x, srcMin.y,
                srcMin.x + srcSiz.x, srcMin.y + srcSiz.y,

                dstMin.x, dstMin.y,
                dstMin.x + dstSiz.x, dstMin.y + dstSiz.y,
    ...
    void Renderer::prepareCameraRender(const JColor32& clearColor, const uint32_t clearFlags, ICamera::CameraRenderData& renderInfo) {
        ...
        _window.resetViewport();
        _window.clear(clearColor, clearFlags);
    }
    ...
    void Renderer::writeCameraToBuffer(Material* blendMaterial, const JColor32& uniformColor, ICamera::CameraRenderData& renderInfo) {
        ...
        if (isValid) {
            ...
            _batch.addVerts(mat, uniformColor, QUAD, 4, QUAD_INDICES, 6);
        ...
        blit:
            _window.updateViewport(dstMin, dstSiz);
            GLCall(glBindFramebuffer(GL_READ_FRAMEBUFFER, renderInfo.frameBuffer->getBufferId()));
            GLCall(glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0));
            GLCall(glBlitFramebuffer(
                srcMin.x, srcMin.y,
                srcMin.x + srcSiz.x, srcMin.y + srcSiz.y,
