...
namespace JEngine {
    namespace Gui {
        ...
        bool drawColor(std::string_view label, VType colorType, void* value, const GUIStyle& style) {
            ...
            bool hasAlpha = false;
            switch (colorType) {
            case JEngine::VType::VTYPE_COLOR32: {
                tmp[3] = float(data[3]) / 255.0f;
                hasAlpha = true;
                __fallthrough;
            case JEngine::VType::VTYPE_COLOR24:
            ...
            case JEngine::VType::VTYPE_COLOR:
        ...
        bool drawBuiltIn(std::string_view label, VType type, void* data, const GUIStyle& style) {
            ...
            case VType::VTYPE_DOUBLE:
                // Note: ImGuiDataType is in the same order for primitive types, 
                // except JEngine's VType starts at 1 because 0 is reserved for VTYPE_NONE
                return drawScalar(label, ImGuiDataType(type) - 1, data, style);
            ...
            case VType::VTYPE_ASSET_REF:
                ...
                // Should allow drag & drop and selecting via a menu/popup (maybe)
                break;
